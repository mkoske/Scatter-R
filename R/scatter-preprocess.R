...
#' @param included.attributes Names or indices of columns/attributes to be included in the processed dataframe (columns that are not in included attributes are stripped; if not given, by default all are included)
#' @param included.classes Names or indices of classes to be included in the processed dataframe (rows where class label is not in included classes are stripped; if not given, by default all are included)
	...
	binarize_and_scale <- function(df, binarized=NULL, scaled=NULL) {
		...
		binarize_factor_df <- function(df) {
			...
			for(i in 1:ncols)
			{
				...
				bdf[,i1[i]:i2[i]] <- m #Insert binarized single variable matrix to dataframe
			}
